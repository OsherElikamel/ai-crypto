...
- **Voting System** — Like/dislike content across all sections, stored in a dedicated votes collection (one row per user per item, enforced by a unique index). Vote status persists across page reloads
...
│   │   ├── models/            # Mongoose schemas (User, Coin, Insight, NewsItem, Meme, Questions, Vote)
│   │   ├── routes/            # Express routers (auth, content, api, quiz, vote)
│   │   ├── services/          # External API integrations (CoinGecko, CoinDesk, Gemini, Reddit) + vote service
│   │   ├── migrations/        # One-time data migrations (embedded vote arrays → votes collection)
